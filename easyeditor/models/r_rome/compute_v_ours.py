...
def compute_v(
    ...
    ##### punish  parallel targets
    parallel_responses = [ " " + p if not p.startswith(" ") else p for p in request["parallel_response"]]

    parallel_target_ids = []
    for p_response in parallel_responses:
        
        p_target_id = tok(p_response, return_tensors="pt", skip_special_tokens=True).to(
            f"cuda:{hparams.device}"
        )["input_ids"][0]
        
        if p_target_id[0] == tok.bos_token_id or p_target_id[0] == tok.unk_token_id:
            p_target_id = p_target_id[1:]
        parallel_target_ids.append(p_target_id)

    # Compile list of rewriting and KL x/y pairs
    rewriting_prompts, kl_prompts = (
        [
            context.format(request["prompt"]) + tok.decode(target_ids[:-1]) #! why [:-1]
            for context in context_templates
        ],
        ["{} is a"],
    )
    
    parallel_prompts = [request["prompt"]+ tok.decode(p_target_id[:-1]) for p_target_id in parallel_target_ids]
    print("======================== Parallel Prompts===================")
    print(parallel_prompts)

    

    all_prompts = rewriting_prompts + parallel_prompts + kl_prompts
    ...
    # Compute parallel targets
    parallel_targets = torch.tensor(-100, device=f"cuda:{hparams.device}").repeat(
        len(parallel_prompts), *input_tok["input_ids"].shape[1:]
    )
    

    # breakpoint()

    for idx, i in enumerate(range( len(rewriting_prompts), len(rewriting_prompts + parallel_prompts))):
        ex_len = input_tok["attention_mask"][i].sum()
        parallel_targets[idx, ex_len - len(parallel_target_ids[idx]) : ex_len] = parallel_target_ids[idx]
    ...
    vanilla_input_prompts = [
        ...
        for context in context_templates
    ] + [ "{}".format(request["prompt"]).format(request["subject"]) for p in parallel_prompts] + [f"{request['subject']} is a"]
    
    # ["{}"] * len(parallel_prompts)
    ...
    target_init, kl_distr_init, parallel_distr_init = None, None, None
    ...
    for it in range(hparams.v_num_grad_steps):
        ...
        # rewrite_target_probs = torch.gather( log_probs[0:rewriting_targets.shape[0]], 2, torch.where(rewriting_targets != -100, rewriting_targets, 0).unsqueeze(2),).squeeze(2)
        rewrite_target_probs = torch.gather( log_probs, 2, torch.where(rewriting_targets != -100, rewriting_targets, 0).unsqueeze(2),).squeeze(2)
        rewrite_mask = (rewriting_targets != -100).float()
        
        # parallel_target_probs = torch.gather( log_probs[-parallel_targets.shape[0]:], 2, torch.where(parallel_targets != -100, parallel_targets, 0).unsqueeze(2),).squeeze(2)
        # parallel_mask = (parallel_targets != -100).float()
        
        
        parallel_log_probs = torch.gather( log_probs[len(rewriting_prompts):len(rewriting_prompts+parallel_prompts)], 2, torch.where(parallel_targets != -100, parallel_targets, 0).unsqueeze(2),).squeeze(2)
        
        select_mask = torch.where(parallel_targets != -100, parallel_targets, 0) != 0
        parallel_target_probs = torch.masked_select(parallel_log_probs, select_mask)
        
        if parallel_distr_init is None:
            parallel_distr_init = parallel_target_probs.detach().clone()
        ...
        #对概率求反
        nll_loss_each = -(rewrite_target_probs * rewrite_mask).sum(1) / target_ids.size(0)
        nll_loss = nll_loss_each.mean()

        # breakpoint()

        parallel_mask = (parallel_targets != -100).float()
        parallel_prob_each = (parallel_log_probs * parallel_mask).sum(1) / torch.tensor([ torch.numel(p_id) for p_id in parallel_target_ids]).to(parallel_mask.device)

        # breakpoint()
        
        prob_token = (rewrite_target_probs * rewrite_mask)[rewrite_mask==1].reshape(-1, target_ids.size(0)).mean(0)
        ...
        parallel_loss = hparams.pkl_factor * torch.nn.functional.kl_div(
            parallel_distr_init, parallel_target_probs, log_target=True, reduction="batchmean"
        )
        ...
        # loss = nll_loss + kl_loss + weight_decay + nll_loss_parallel
        loss = nll_loss + kl_loss + weight_decay + parallel_loss
        # breakpoint()

        print(
            f"loss {np.round(loss.item(), 3)} = {np.round(nll_loss.item(), 3)} + {np.round(kl_loss.item(), 3)} + {np.round(weight_decay.item(), 3)} + {np.round(parallel_loss.item(), 3)}"
            f"avg prob of [{request['target_new']}] "
            f"{torch.exp(-nll_loss_each).mean().item()} "
            # f"individual prob of [{request['target_new']}] "
            # f"{torch.exp(prob_token)}"
            f"avg prob of parallel response",
            f"{torch.exp(parallel_prob_each)}"
        )
        breakpoint()
        # breakpoint()
        if loss < 5e-2:
